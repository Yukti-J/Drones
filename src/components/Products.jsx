...
const Products = () => {
  ...
  return (
    <div className="mt-24">
      <h1 className="text-5xl text-center my-4 font-rubik">Products</h1>
      <div className="grid grid-cols-3 w-4/5 m-auto">
        {/* Card that has product image, name, price, and description */}
        {limitedProducts.map((product) => (
          <div
            key={product.id}
            className="my-5 mx-10 bg-white rounded-xl drop-shadow-2xl transition ease-in-out duration-300	 hover:scale-105 "
          >
            <img
              src={product.image}
              alt={product.name}
            ...
            <p className="text-center font-rubik mb-2 text-gray-700 px-5">
              {product.description}
            </p>
